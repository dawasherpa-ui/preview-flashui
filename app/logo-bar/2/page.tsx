...
const logos = [
  { name: 'Vercel', url: 'https://res.cloudinary.com/dfhp33ufc/image/upload/v1715881430/vercel_wordmark_dark_mhv8u8.svg', blackFilter: true },
  { name: 'Nextjs', url: 'https://res.cloudinary.com/dfhp33ufc/image/upload/v1715881475/nextjs_logo_dark_gfkf8m.svg', blackFilter: true },
            ...
            {[...logos, ...logos].map((logo, index) => (
              <div key={index} className="flex-shrink-0 w-40">
                <img
                  src={logo.url}
                  alt={`${logo.name} logo`}
                  className={`h-12 w-auto object-contain ${logo.blackFilter ? 'brightness-0' : ''}`}
                />
              </div>
            ))}
          </div>
